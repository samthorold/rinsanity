...
Both channels are **wired into the engine**. Each insured accumulates its loss record year by year — the asset burns whether or not it bought cover, and either way the year is evidence the broker presents next renewal — and the record rides onto the submission every syndicate quotes. The modifier scopes to the **attritional** ELF only: applying an attritionally-derived modifier to the cat ELF would experience-update a model-anchored loss cost, which is the one thing the tail regime forbids. On the other channel each syndicate accumulates its own exposure-years and what that book burnt, so `own` and `n` in the blend are earned rather than assumed. The `cycle` emission carries `pool_burning_cost` — the burning-cost relativity of the pool the market took on, weighted by the attritional exposure it took — and `insured_declines`, so the self-selection can be read; pinning `credibility_k` very high is the control arm that rates nothing. Measured over 300-year runs across eight seeds, the reference market's rated pool burns about **1% less** than the same market with rating switched off, in the right direction on six of the eight, but it is **not** measurably below the population it was offered. Three reasons, all structural rather than incidental: the market runs soft (firm orders around 0.6 of technical premium and roughly half of WTP, leaving no headroom to price anyone out); the attritional ELF the modifier scopes to is only about an eighth of a cat-dominated loss cost, so a chronic risk pays a few per cent more rather than a multiple; and `restructure_tower` sheds the *worst value-for-money* band first, which is the surcharged attritional band — so a priced-out risk retains its own attritional losses and keeps the cat layer, rather than declining outright. The mechanism is causally live and correctly signed, and where price sits close to WTP (a less risk-averse population, hazards dispersed wider) the gap opens and holds its sign across seeds. Delivering self-selection at the reference calibration is a calibration question, not a mechanism one.
