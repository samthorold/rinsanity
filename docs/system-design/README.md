...
**The attractor and its direction are two separate claims, and they are read separately.** *That* the distribution settles — its mean stationary between successive half-centuries, its width bounded rather than diffusing, and materially tighter than the same market replenishing entrants from the fixed prior — is the robust finding: it has stood unchanged across every trajectory-moving change made since it was written (causally live price herding, Lloyd's-scale panel granularity, the seed-panel discipline, experience rating). It is asserted over a single long trajectory because it is a claim about where *a* population ends up, and it is cheap relative to its value.

*Which way* selection pushes a trait is a different claim and needs a different instrument. It is a claim about a **direction**, so it is read over a seed panel fixed in advance, on a shorter horizon, counting how many seeds agree on the sign — never on the size of a margin on one trajectory, which is how the claim previously drifted from the hurdle rate to the reserving bias and then onto a margin thin enough to renegotiate every time the market changed. The direction that survives that instrument is on the **reserving bias**: with selection acting, the population settles materially more conservative than replenishment from the prior sustains, because under-reserving is culled. The trap the split exposes is worth stating: the single seed the claim used to be asserted on turns out to carry by far the *smallest* gap in the panel — the claim was near-failing on the one trajectory it was read on while holding comfortably on every other, which is exactly the failure mode single-seed statistics produce in this market.
