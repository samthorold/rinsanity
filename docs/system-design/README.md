...
| 3 | Broker–syndicate herding | A derived weight `w` concedes a follower's reservation price toward a reputable lead's firm order (price, never belief) | *Lead, follow, and herding* |
...
The panel forms by **firm-order subscription**. The lead's `TP · AvT` is the layer's **firm order** — the single price the insured pays. A follower **subscribes when the firm order clears its herding-adjusted reservation price** — `firm_order ≥ own(TP · AvT) · (1 − w · concession)` — and its exposure limits permit, taking a share; the panel fills capacity-first up to the layer limit, and an unfilled remainder leaves the layer **partially placed** (the insured restructures or retains the gap). The insured pays one price per layer, never a blend of the panellists' quotes. The weight `w` moves the **threshold**, not the quote: it measures how far below its own technical view a syndicate will write at a reputable lead's terms. This distinction is load-bearing — anchoring the *quote* and then testing it against the lead's own price is degenerate, since a convex combination of `own` and `lead` sits at or below `lead` exactly when `own` does, for any `w < 1`. The blend `(1 − w)·own + w·lead` remains the right expression of the follower's *view*; it is not the right expression of its *decision*. This is why herding is load-bearing in *formation*, not only in the quote: a follower anchored toward a reputable lead lowers its threshold and **subscribes to a firm order it would otherwise reject** — the channel by which a reputable lead's mispricing propagates across the panel (#3). A competing-auction alternative — stacking quotes cheapest-first and clearing at the marginal bid — is rejected: it reduces followers to price-takers in an order book, sidelining the subscription-herding the panel exists to express, and is more auction-like than the subscription market it models. Price competition is **across** panels — a syndicate leads one placement and follows another, and the insured takes the cheapest assembled panel within WTP (see *Demand and clearing*) — not an intra-layer auction.
...
**Cat-model homogeneity as systemic risk (#14)** emerges from shared *error against the truth*. Two market-level knobs govern it: the **heterogeneity spread** of syndicates' model parameters and a possible **shared bias** (a common offset from the true process). Biased models make every syndicate underprice the same tail and fail together — correlated insolvency from a structural cause. The systemic ingredient is the **shared bias, not the agreement**: measured over the reference market, calibrated populations produced 0 failures in 16 runs and biased populations 15 in 16, while a *homogeneous but calibrated* population never failed at all. Homogeneity alone is harmless. Scatter is not a remedy either — dispersing beliefs around a biased centre preserves the mean error, so heterogeneous-biased populations fail at rates indistinguishable from homogeneous-biased ones. Only recalibration of the centre removes the hazard; heterogeneity governs the *timing* and clustering of failures rather than whether they occur. The bias required is large — the reference market needs to believe catastrophe costs roughly an eighth of the truth before insolvency is reliable — which is itself a measurement of how far that market sits from the edge. #14 is therefore a controlled experiment in the spread/bias knobs.
